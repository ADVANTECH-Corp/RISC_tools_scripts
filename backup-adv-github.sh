...
GHBU_ORG=${GHBU_ORG-ADVANTECH-Corp}                                   # the GitHub organization whose repos will be backed up
...
REPOLIST=`check curl --silent -u $GHBU_UNAME:$GHBU_PASSWD ${GHBU_API}/orgs/${GHBU_ORG}/repos\?per_page=130 -q | check grep ^"    \"name\"" | check awk -F': "' '{print $2}' | check sed -e 's/",//g'`
...
#REPOLIST=`check curl --silent -u $GHBU_UNAME:$GHBU_PASSWD ${GHBU_API}/users/$GHBU_UNAME/repos -q | check grep -v "Apache" |check grep "\"name\"" | check awk -F': "' '{print $2}' | check sed -e 's/",//g'`
